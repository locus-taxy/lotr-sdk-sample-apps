...
@IBDesignable open class CBPinEntryView: UIView {
    ...
    fileprivate var entryButtons = [UIButton]()
    ...
    @objc private func didPressCodeButton(_: UIButton) {
        errorMode = false
    ...
    open func setPinAsString(initialValue: String) {
        textField.text = initialValue
        for (index, char) in initialValue.enumerated() {
            let buttons = entryButtons.filter { $0.tag == index + 1 }
...
extension UIButton {
    func addBottomBorder(thickness: CGFloat, color: UIColor, cornerRadius _: CGFloat = 8) {
